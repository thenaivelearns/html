...
(function() {
	 ...
	 * @returns {WfCanvas}
	 */
	function WfCanvas(canvas, xmlStr, lytStr, currentSteps, historySteps, historyConnecters) {
		currentSteps = currentSteps || [];
		historySteps = historySteps || [];
		historyConnecters = historyConnecters || [];
		...
		var 
			...
			fontColor = "Black",
			borderColor = "Black",
			//stepColor = "#FFFF37",//yellow
            stepColor = "#eee"
			lineColor = "red",
			shadowOffset = 2;
			initActionCorlor = "#68a5c7",//blue
			currentStepColor = "#FF5151",//red
			historyStepColor = "#68a5c7",//blue
			shadwColor = "#aaa",
			ctx = null,
			xml = null,
			lyt = null,
		...
		function init() {
			...
			$.each(connectorNodeMap, function(id, connectorNode){
				...
				//历史连线标红
				$.each(historyConnecters, function(idx, connecterId){
					if(id==connecterId){
						isPassby = true;
					}
				});
				
				var targetId = xmlNode.attr("step");
				connectorMap[id] = {
					id: id,
		...
		function drawCell(cell) {
			...
			if (cell.type == "InitialActionCell") {
				drawRect20(cell, cell.width, cell.height);
			}else{
		 ...
		 * @param drawShadow 是否画阴影
		 */
		function drawRect20(p, w, h, fill, stroke, drawShadow) {
			fill = typeof(fill) == "undefined" ? true : fill;
			//stroke = typeof(stroke) == "undefined" ? true : stroke;
			drawShadow = typeof(drawShadow) == "undefined" ? true : drawShadow;
			if (drawShadow) {
				...
				var oldStyle = ctx.fillStyle;
				ctx.fillStyle = shadwColor;
				drawRect20({x: p.x + offset, y: p.y + offset}, w, h, true, false, false);
				ctx.fillStyle = oldStyle;
			}
	...
	window.drawWf = function(canvas, xmlStr, lytStr, currentSteps, historySteps, historyConnecters) {
		var wf = new WfCanvas(canvas, xmlStr, lytStr, currentSteps, historySteps, historyConnecters);
